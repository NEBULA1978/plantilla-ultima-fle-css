...
read -p "Introduce el nombre de usuario de MySQL: " usuario
read -s -p "Introduce la contraseña de MySQL: " contrasena
echo # Imprimir una nueva línea
read -p "Introduce el nombre de la base de datos: " basededatos
    ...
    case $sql_option in
        ...
        5)
            ...
            case $optimization_option in
                1)
                    # Opción de índices
                    echo "CREATE INDEX idx_nomina ON ingeniero (nomina);" | mysql -u "$usuario" -p"$contrasena" -D "$basededatos"
                    ;;
                2)
